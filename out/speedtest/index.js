...
var ASM_CONSTS = {
  72540: function() {runtimeKeepalivePush();},  
 72564: function() {runtimeKeepalivePop();},  
 72587: function() {console.log('Calling emscripten_init_pthreadfs() is no longer necessary');},  
 72662: function($0, $1) {(async() => { console.log(`Loading package ${UTF8ToString($1)}`); importScripts(UTF8ToString($1)); await PThreadFS.loadAvailablePackages(); wasmTable.get($0)(); })();}
};
  ...
  var MEMFS_ASYNC = {ops_table:null,mount:function(mount) {
        return MEMFS_ASYNC.createNode(null, '/', 16384 | 511 /* 0777 */, 0);
      },createNode:function(parent, name, mode, dev) {
        if (PThreadFS.isBlkdev(mode) || PThreadFS.isFIFO(mode)) {
          // no supported
          throw new PThreadFS.ErrnoError(63);
        }
        ...
        if (newSize == 0) {
          ...
          throw PThreadFS.genericErrors[44];
        },mknod:function(parent, name, mode, dev) {
          ...
          if (PThreadFS.isDir(old_node.mode)) {
            ...
            if (new_node) {
              for (var i in new_node.contents) {
                throw new PThreadFS.ErrnoError(55);
              }
            }
          }
          // do the internal rewiring
          ...
          var node = MEMFS_ASYNC.createNode(parent, newname, 511 /* 0777 */ | 40960, 0);
          node.link = oldpath;
          return node;
        },readlink:function(node) {
          if (!PThreadFS.isLink(node.mode)) {
            throw new PThreadFS.ErrnoError(28);
          }
          return node.link;
        }},stream_ops:{read:function(stream, buffer, offset, length, position) {
          var contents = stream.node.contents;
          ...
          if (!PThreadFS.isFile(stream.node.mode)) {
            throw new PThreadFS.ErrnoError(43);
          }
          var ptr;
          var allocated;
          var contents = stream.node.contents;
          ...
          if (!(flags & 2) && contents.buffer === buffer) {
            ...
            ptr = mmapAlloc(length);
            if (!ptr) {
              throw new PThreadFS.ErrnoError(48);
            }
            HEAP8.set(contents, ptr);
          }
          return { ptr: ptr, allocated: allocated };
  ...
  var PThreadFS = {root:null,mounts:[],devices:{},streams:[],nextInode:1,nameTable:null,currentPath:"/",initialized:false,ignorePermissions:true,trackingDelegate:{},tracking:{openFlags:{READ:1,WRITE:2}},ErrnoError:null,genericErrors:{},filesystems:null,syncFSRequests:0,lookupPath:async function(path, opts = {}) {
        ...
        if (opts.recurse_count > 8) {  // max recursive lookup of 8
          throw new PThreadFS.ErrnoError(32);
        }
      ...
      },MIN_FD:4097,MAX_OPEN_FDS:4096,nextfd:function(fd_start, fd_end) {
        ...
        throw new PThreadFS.ErrnoError(33);
        ...
        PThreadFS.streams[fd] = null;
          ...
          throw new PThreadFS.ErrnoError(70);
        }},major:function(dev) {
        return ((dev) >> 8);
      },minor:function(dev) {
        return ((dev) & 0xff);
        ...
        if (root && PThreadFS.root) {
          throw new PThreadFS.ErrnoError(10);
        } else if (!root && !pseudo) {
          ...
          if (!PThreadFS.isDir(node.mode)) {
            throw new PThreadFS.ErrnoError(54);
          }
        }
        ...
        if (!PATH_FS.resolve(oldpath)) {
          throw new PThreadFS.ErrnoError(44);
        }
        ...
        if (!old_dir || !new_dir) throw new PThreadFS.ErrnoError(44);
        // need to be part of the same mount
        if (old_dir.mount !== new_dir.mount) {
          throw new PThreadFS.ErrnoError(75);
        }
        // source must exist
        ...
        if (!stream) {
          throw new PThreadFS.ErrnoError(8);
        }
        ...
        if (PThreadFS.isDir(node.mode)) {
          throw new PThreadFS.ErrnoError(31);
        }
        ...
        if ((flags & 64)) {
          if (node) {
            // if O_CREAT and O_EXCL are set, error out if the node already exists
            if ((flags & 128)) {
              throw new PThreadFS.ErrnoError(20);
            }
          } else {
            // node doesn't exist, try to create it
        ...
        // if asked only for a directory, then this must be one
        if ((flags & 65536) && !PThreadFS.isDir(node.mode)) {
          throw new PThreadFS.ErrnoError(54);
        }
        // check permissions, if this is not a file we just created now (it is ok to
        // create and write to a file with read-only permissions; it is read-only
        ...
        if (stream.seekable && stream.flags & 1024) {
          // seek to the end before writing in append mode
          await PThreadFS.llseek(stream, 0, 2);
        }
        var seeking = typeof position !== 'undefined';
        if (!seeking) {
          position = stream.position;
        ...
        await PThreadFS.mount({
          mount: function() {
            var node = PThreadFS.createNode(proc_self, 'fd', 16384 | 511 /* 0777 */, 73);
            node.node_ops = {
              lookup: function(parent, name) {
                var fd = +name;
                var stream = PThreadFS.getStream(fd);
                if (!stream) throw new PThreadFS.ErrnoError(8);
                var ret = {
                  parent: null,
                  mount: { mountpoint: 'fake' },
        ...
        if (data) {
          ...
          // make sure we can write to the file
          await PThreadFS.chmod(node, mode | 146);
          var stream = await PThreadFS.open(node, 577);
          ...
          read: function(stream, buffer, offset, length, pos /* ignored */) {
            ...
            for (var i = 0; i < length; i++) {
              ...
              try {
                ...
                throw new PThreadFS.ErrnoError(29);
              }
              if (result === undefined && bytesRead === 0) {
                throw new PThreadFS.ErrnoError(6);
              }
              if (result === null || result === undefined) break;
              bytesRead++;
              buffer[offset+i] = result;
  ...
  var ASYNCSYSCALLS = {mappings:{},DEFAULT_POLLMASK:5,calculateAt:async function(dirfd, path, allowEmpty) {
        if (path[0] === '/') {
          return path;
        }
        // relative path
        ...
        if (dirfd === -100) {
          ...
          if (!dirstream) throw new PThreadFS.ErrnoError(8);
          dir = dirstream.path;
        }
        if (path.length == 0) {
          if (!allowEmpty) {
        ...
        for (var i = 0; i < iovcnt; i++) {
          ...
          var curr = await PThreadFS.read(stream, HEAP8,ptr, len, offset);
          if (curr < 0) return -1;
          ret += curr;
        ...
        for (var i = 0; i < iovcnt; i++) {
          var ptr = HEAP32[(((iov)+(i*8))>>2)];
          var len = HEAP32[(((iov)+(i*8 + 4))>>2)];
          var curr = await PThreadFS.write(stream, HEAP8,ptr, len, offset);
          if (curr < 0) return -1;
          ret += curr;
        }
        return ret;
        ...
        var ret = HEAP32[(((ASYNCSYSCALLS.varargs)-(4))>>2)];
        return ret;
      },getStr:function(ptr) {
  ...
  async function _fd_close_async(fd, iov, iovcnt, pnum) {try {
  ...
  var FSAFS = {createSyncAccessHandle:async function(node) {
        ...
        let node = FSAFS.createNode(null, '/', 16384 | 511 /* 0777 */, 0);
        FSAFS.root = await navigator.storage.getDirectory();
        node.localReference = FSAFS.root;
        return node;
      },node_ops:{getattr:async function(node) {
          var attr = {};
          // device numbers reuse inode numbers.
          attr.dev = PThreadFS.isChrdev(node.mode) ? node.id : 1;
          attr.ino = node.id;
          ...
          if (attr.size !== undefined) {
            ...
            throw PThreadFS.genericErrors[10];
          }
        },lookup:async function (parent, name) {
          ...
          throw new PThreadFS.ErrnoError(75);
        }},stream_ops:{open:async function (stream) {
          if (PThreadFS.isDir(stream.node.mode)) {
          ...
          if (!PThreadFS.isFile(stream.node.mode)) {
            ...
            throw new PThreadFS.ErrnoError(52);
          }
  ...
  function ___fd_close_async(fd,resume) {_fd_close_async(fd).then((res) => {wasmTable.get(resume)(res);});}

  async function _fd_read_async(fd, iov, iovcnt, pnum) {try {
  ...
  function ___fd_read_async(fd,iovs,iovs_len,nread,resume) {_fd_read_async(fd,iovs,iovs_len,nread).then((res) => {wasmTable.get(resume)(res);});}

  async function _fd_seek_async(fd, offset_low, offset_high, whence, newOffset) {try {
  ...
  function ___fd_seek_async(fd,offset_low, offset_high,whence,newOffset,resume) {_fd_seek_async(fd,offset_low, offset_high,whence,newOffset).then((res) => {wasmTable.get(resume)(res);});}

  async function _fd_sync_async(fd) {try {
  ...
  function ___fd_sync_async(fd,resume) {_fd_sync_async(fd).then((res) => {wasmTable.get(resume)(res);});}

  async function _fd_write_async(fd, iov, iovcnt, pnum) {try {
  ...
  function ___fd_write_async(fd,iovs,iovs_len,nwritten,resume) {_fd_write_async(fd,iovs,iovs_len,nwritten).then((res) => {wasmTable.get(resume)(res);});}
  ...
  async function _access_async(path, amode) {try {
  ...
  function ___sys_access_async(path,amode, resume) {_access_async(path,amode).then((res) => {wasmTable.get(resume)(res);});}

  async function _fcntl64_async(fd, cmd, varargs) {ASYNCSYSCALLS.varargs = varargs;
  try {
  ...
  function ___sys_fcntl64_async(fd,cmd,varargs, resume) {_fcntl64_async(fd,cmd,varargs).then((res) => {wasmTable.get(resume)(res);});}

  async function _fstat64_async(fd, buf) {try {
  ...
  function ___sys_fstat64_async(fd,buf, resume) {_fstat64_async(fd,buf).then((res) => {wasmTable.get(resume)(res);});}

  async function _ftruncate64_async(fd, low, high) {try {
  ...
  function ___sys_ftruncate64_async(fd,low,high, resume) {_ftruncate64_async(fd,low,high).then((res) => {wasmTable.get(resume)(res);});}

  async function _lstat64_async(path, buf) {try {
  ...
  function ___sys_lstat64_async(path,buf, resume) {_lstat64_async(path,buf).then((res) => {wasmTable.get(resume)(res);});}

  async function _open_async(path, flags, mode) {try {
  ...
  function ___sys_open_async(path,flags,varargs, resume) {_open_async(path,flags,varargs).then((res) => {wasmTable.get(resume)(res);});}

  async function _stat64_async(path, buf) {try {
  ...
  function ___sys_stat64_async(path,buf, resume) {_stat64_async(path,buf).then((res) => {wasmTable.get(resume)(res);});}

  async function _unlink_async(path) {try {
  ...
  function ___sys_unlink_async(path, resume) {_unlink_async(path).then((res) => {wasmTable.get(resume)(res);});}
  ...
  function ___syscall_fstatat64(dirfd, path, buf, flags) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(5, 1, dirfd, path, buf, flags);
  ...
  function ___syscall_ftruncate64(fd, low, high) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(6, 1, fd, low, high);
  ...
  function ___syscall_getcwd(buf, size) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(7, 1, buf, size);
  ...
  function ___syscall_mmap2(addr, len, prot, flags, fd, off) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(9, 1, addr, len, prot, flags, fd, off);
  ...
  function ___syscall_munmap(addr, len) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(10, 1, addr, len);
  ...
  function _tzset_impl(timezone, daylight, tzname) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(14, 1, timezone, daylight, tzname);
  ...
  function _emscripten_set_canvas_element_size_main_thread(target, width, height) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(15, 1, target, width, height);
  ...
  function _environ_get(__environ, environ_buf) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(16, 1, __environ, environ_buf);
  ...
  function _environ_sizes_get(penviron_count, penviron_buf_size) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(17, 1, penviron_count, penviron_buf_size);
  ...
  function _fd_close(fd) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(18, 1, fd);
  ...
  function _fd_sync(fd) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(21, 1, fd);
  ...
  function _fd_write(fd, iov, iovcnt, pnum) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(22, 1, fd, iov, iovcnt, pnum);
  ...
  function _pthreadfs_init(folder_ref, resume) {
  ...
  function _utimes(path, times) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(23, 1, path, times);
...
var proxiedFunctionTable = [null,exitOnMainThread,___syscall_access,___syscall_fcntl64,___syscall_fstat64,___syscall_fstatat64,___syscall_ftruncate64,___syscall_getcwd,___syscall_lstat64,___syscall_mmap2,___syscall_munmap,___syscall_open,___syscall_stat64,___syscall_unlink,_tzset_impl,_emscripten_set_canvas_element_size_main_thread,_environ_get,_environ_sizes_get,_fd_close,_fd_read,_fd_seek,_fd_sync,_fd_write,_utimes];
...
var asmLibraryArg = {
  ...
  "__fd_close_async": ___fd_close_async,
  "__fd_read_async": ___fd_read_async,
  "__fd_seek_async": ___fd_seek_async,
  "__fd_sync_async": ___fd_sync_async,
  "__fd_write_async": ___fd_write_async,
  "__pthread_create_js": ___pthread_create_js,
  "__sys_access_async": ___sys_access_async,
  "__sys_fcntl64_async": ___sys_fcntl64_async,
  "__sys_fstat64_async": ___sys_fstat64_async,
  "__sys_ftruncate64_async": ___sys_ftruncate64_async,
  "__sys_lstat64_async": ___sys_lstat64_async,
  "__sys_open_async": ___sys_open_async,
  "__sys_stat64_async": ___sys_stat64_async,
  "__sys_unlink_async": ___sys_unlink_async,
  ...
  "memory": wasmMemory,
  "pthreadfs_init": _pthreadfs_init,
...
var _emscripten_proxy_main = Module["_emscripten_proxy_main"] = function() {
  return (_emscripten_proxy_main = Module["_emscripten_proxy_main"] = Module["asm"]["emscripten_proxy_main"]).apply(null, arguments);
};
...
var __emscripten_allow_main_runtime_queued_calls = Module['__emscripten_allow_main_runtime_queued_calls'] = 72220;
